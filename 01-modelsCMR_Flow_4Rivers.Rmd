...
if (rerunSurivalModels) {
  ...
  hmm.phiT_pT_psiT_DHMM <- nimbleCode({
      ...
      for (t in 1:(T-1)){ # loop over time
          for (r in 1:(nRivers - 1)){
              lpsiA[r,t] ~ dnorm(0, sd = 1000)  # sd too big?
              lpsiB[r,t] ~ dnorm(0, sd = 1000)
              lpsiC[r,t] ~ dnorm(0, sd = 1000)
              lpsiD[r,t] ~ dnorm(0, sd = 1000)
             
              # constrain the transitions such that their sum is < 1
              psiA[r,t] <- exp(lpsiA[r,t]) / (1 + exp(lpsiA[1,t]) + exp(lpsiA[2,t]) + exp(lpsiA[3,t]))
              psiB[r,t] <- exp(lpsiB[r,t]) / (1 + exp(lpsiB[1,t]) + exp(lpsiB[2,t]) + exp(lpsiB[3,t]))
              psiC[r,t] <- exp(lpsiC[r,t]) / (1 + exp(lpsiC[1,t]) + exp(lpsiC[2,t]) + exp(lpsiC[3,t]))
              psiD[r,t] <- exp(lpsiD[r,t]) / (1 + exp(lpsiD[1,t]) + exp(lpsiD[2,t]) + exp(lpsiD[3,t]))
          }
         
          # last transition probability
          psiA[4,t] <- 1 - psiA[1,t] - psiA[2,t] - psiA[3,t]
          psiB[4,t] <- 1 - psiB[1,t] - psiB[2,t] - psiB[3,t]
          psiC[4,t] <- 1 - psiC[1,t] - psiC[2,t] - psiC[3,t]
          psiD[4,t] <- 1 - psiD[1,t] - psiD[2,t] - psiD[3,t]
       
          for (c in 1:nCohorts){
              gamma[1,1,t,c] <- betaPhi[1,c,t] * psiA[1,t]
              gamma[1,2,t,c] <- betaPhi[1,c,t] * psiA[2,t]
              gamma[1,3,t,c] <- betaPhi[1,c,t] * psiA[3,t]
              gamma[1,4,t,c] <- betaPhi[1,c,t] * psiA[4,t]
              gamma[1,5,t,c] <- 1 - betaPhi[1,c,t]
              gamma[2,1,t,c] <- betaPhi[2,c,t] * psiB[1,t]
              gamma[2,2,t,c] <- betaPhi[2,c,t] * psiB[2,t]
              gamma[2,3,t,c] <- betaPhi[2,c,t] * psiB[3,t]
              gamma[2,4,t,c] <- betaPhi[2,c,t] * psiB[4,t]
              gamma[2,5,t,c] <- 1 - betaPhi[2,c,t]
              gamma[3,1,t,c] <- betaPhi[3,c,t] * psiC[1,t]
              gamma[3,2,t,c] <- betaPhi[3,c,t] * psiC[2,t]
              gamma[3,3,t,c] <- betaPhi[3,c,t] * psiC[3,t]
              gamma[3,4,t,c] <- betaPhi[3,c,t] * psiC[4,t]
              gamma[3,5,t,c] <- 1 - betaPhi[3,c,t]
              gamma[4,1,t,c] <- betaPhi[4,c,t] * psiD[1,t]
              gamma[4,2,t,c] <- betaPhi[4,c,t] * psiD[2,t]
              gamma[4,3,t,c] <- betaPhi[4,c,t] * psiD[3,t]
              gamma[4,4,t,c] <- betaPhi[4,c,t] * psiD[4,t]
              gamma[4,5,t,c] <- 1 - betaPhi[4,c,t]
              gamma[5,1,t,c] <- 0
              gamma[5,2,t,c] <- 0
              gamma[5,3,t,c] <- 0
              gamma[5,4,t,c] <- 0
  ...
  initialValues <- function()
      list(
          ...
          betaPhi = array(runif(nRivers * nCohorts * (myConstants$T - 1), 0, 1), c(nRivers, nCohorts, (myConstants$T - 1))),
          betaPRiver = array(runif(nRivers, 0, 1), c(nRivers)),
          betaPRiverCohort = array(runif(nRivers * nCohorts, 0, 1), c(nRivers, nCohorts)),
          betaP = array(runif(nRivers * nCohorts * (myConstants$T - 1), 0, 1), c(nRivers, nCohorts, (myConstants$T - 1))),
          ...
          lpsiA = array(rnorm((nRivers - 1) * (myConstants$T - 1), 0, 1), c((nRivers - 1), (myConstants$T - 1))),
          lpsiB = array(rnorm((nRivers - 1) * (myConstants$T - 1), 0, 1), c((nRivers - 1), (myConstants$T - 1))),
          lpsiC = array(rnorm((nRivers - 1) * (myConstants$T - 1), 0, 1), c((nRivers - 1), (myConstants$T - 1))),
          lpsiD = array(rnorm((nRivers - 1) * (myConstants$T - 1), 0, 1), c((nRivers - 1), (myConstants$T - 1)))
          ####z = zInitsNA
      )
...
Phi and p and psi vary by time    
Includes transition probabilities among rivers
Add cohort (c) indexing to psi's
...
if (rerunSurivalModels) {
  ...
  hmm.phiT_pT_psiT_DHMM <- nimbleCode({
      ...
      for (t in 1:(T-1)){ # loop over time
        for (c in 1:nCohorts){
          for (r in 1:(nRivers - 1)){
              lpsiA[r,t,c] ~ dnorm(0, sd = 1000)
              lpsiB[r,t,c] ~ dnorm(0, sd = 1000)
              lpsiC[r,t,c] ~ dnorm(0, sd = 1000)
              lpsiD[r,t,c] ~ dnorm(0, sd = 1000)
          ...
          psiD[4,t,c] <- 1 - psiD[1,t,c] - psiD[2,t,c] - psiD[3,t,c]
              ...
              gamma[1,1,t,c] <- betaPhi[1,c,t] * psiA[1,t,c]
              gamma[1,2,t,c] <- betaPhi[1,c,t] * psiA[2,t,c]
              gamma[1,3,t,c] <- betaPhi[1,c,t] * psiA[3,t,c]
              gamma[1,4,t,c] <- betaPhi[1,c,t] * psiA[4,t,c]
              gamma[1,5,t,c] <- 1 - betaPhi[1,c,t]
              gamma[2,1,t,c] <- betaPhi[2,c,t] * psiB[1,t,c]
              gamma[2,2,t,c] <- betaPhi[2,c,t] * psiB[2,t,c]
              gamma[2,3,t,c] <- betaPhi[2,c,t] * psiB[3,t,c]
              gamma[2,4,t,c] <- betaPhi[2,c,t] * psiB[4,t,c]
              gamma[2,5,t,c] <- 1 - betaPhi[2,c,t]
              gamma[3,1,t,c] <- betaPhi[3,c,t] * psiC[1,t,c]
              gamma[3,2,t,c] <- betaPhi[3,c,t] * psiC[2,t,c]
              gamma[3,3,t,c] <- betaPhi[3,c,t] * psiC[3,t,c]
              gamma[3,4,t,c] <- betaPhi[3,c,t] * psiC[4,t,c]
              gamma[3,5,t,c] <- 1 - betaPhi[3,c,t]
              gamma[4,1,t,c] <- betaPhi[4,c,t] * psiD[1,t,c]
              gamma[4,2,t,c] <- betaPhi[4,c,t] * psiD[2,t,c]
              gamma[4,3,t,c] <- betaPhi[4,c,t] * psiD[3,t,c]
              gamma[4,4,t,c] <- betaPhi[4,c,t] * psiD[4,t,c]
              gamma[4,5,t,c] <- 1 - betaPhi[4,c,t]
              gamma[5,1,t,c] <- 0
              gamma[5,2,t,c] <- 0
              gamma[5,3,t,c] <- 0
...
Includes transition probabilities among rivers
Add cohort (c) indexing to psi's
Fix c,t indexing, change to t,c
...
if (rerunSurivalModels) {
  ...
  hmm.phiT_pT_psiT_DHMM <- nimbleCode({
      ...
      for (t in 1:(T-1)){ # loop over time
        for (c in 1:nCohorts){
          for (r in 1:(nRivers - 1)){
              lpsiA[r,t,c] ~ dnorm(0, sd = 1000) # sd too big???
              lpsiB[r,t,c] ~ dnorm(0, sd = 1000)
              lpsiC[r,t,c] ~ dnorm(0, sd = 1000)
              lpsiD[r,t,c] ~ dnorm(0, sd = 1000)
