...
getEH_AIS <- function(dIn, cols, ops, vals, maxOccasionValue, maxIndexByCohort = 1E10){
  ...
  # limit data to first 'maxIndexByCohort' individuals
  d <- d %>%
    group_by(cohort) %>%
  ...
  isYOYWide <- getEHDataWide_AIS(d, cols, ops, vals, "isYOY", maxOccasionValue, valuesFill = NA)
  isYOYMatrix <- as.matrix(isYOYWide %>% select(-tag), nrow = nrow(riverWide), ncol = ncol(riverWide) - 1)
  ...
  eh <- getEH_AIS(cdWB_CMR0, cols, ops, vals, maxAgeInSamples) #, maxIndexByCohort = 200)
