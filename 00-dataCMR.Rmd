...
getEHDataWide <- function(d, cols, ops, vals, var, occasionVar = "ageInSamples", maxOccasionValue, valuesFill = 0){   
  d %>%
    ...
    pivot_wider(
      ...
      names_prefix = substr(eval(substitute(occasionVar)), 1, 1), #"s",
...
getEH <- function(d, cols, ops, vals, occasionVar = "ageInSamples", maxOccasionValue){
  ...
  flowWide <- getEHDataWide(d, cols, ops, vals, "meanFlow", occasionVar, maxOccasionValue, valuesFill = NA)
  flowMatrix <- as.matrix(flowWide %>% select(-tag), nrow = nrow(flowWide), ncol = ncol(flowWide) - 1)
  
  temperatureWide <- getEHDataWide(d, cols, ops, vals, "meanTemperature", occasionVar, maxOccasionValue, valuesFill = NA)
  ...
  seasons <- tags %>% left_join(data %>% select(tag, season) %>% unique()) %>% select(season)
  
  return(list(eh = eh, flow = flowMatrix, temperature = temperatureMatrix, tags = tags, cohorts = cohorts, seasons = seasons, data = data ))
}
