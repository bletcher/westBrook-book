...
getEHDataWide <- function(d, cols, ops, vals, var, occasionVar = "ageInSamples", valuesFill = 0){   
  
  out <- d %>%
    ehFilter(cols, ops, vals) %>% 
    filter(ageInSamples > 0) %>%
...
getEH <- function(d, cols, ops, vals, occasionVar = "ageInSamples"){
  encWide <- getEHDataWide(d, cols, ops, vals, "enc", occasionVar)
  eh <- as.matrix(encWide %>% select(-tag), nrow = nrow(encWide), ncol = ncol(encWide) - 1)
  
  flowWide <- getEHDataWide(d, cols, ops, vals, "meanFlow", occasionVar)
  flowMatrix <- as.matrix(flowWide %>% select(-tag), nrow = nrow(flowWide), ncol = ncol(flowWide) - 1)
  
  temperatureWide <- getEHDataWide(d, cols, ops, vals, "meanTemperature", occasionVar)
...
if (getNew_encounterHistories) {
  ...
  eh <- getEH(cdWB_CMR0, cols, ops, vals, occasionVar = "sampleNumber")
  ...
  eh <- getEH(cdWB_CMR0, cols, ops, vals, occasionVar = "ageInSamples")
  fileName = paste0("eh_", stri_paste_list(vals, collapse = "_"))
  save(eh, file = paste0('./models/dataOut/', fileName, '.RData'))
