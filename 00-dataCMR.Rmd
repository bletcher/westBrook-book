...
getEH_AIS <- function(dIn, cols, ops, vals, maxOccasionValue, maxIndexByCohort = 1E10){
  ...
  cols <- list("cohort")
  ops <-  list("%in%")
  vals <- list(2002:2014)#list(2002:2014)
  #vals <- list(2002:2014)
                                                               # only include first x fish per cohort
  eh <- getEH_AIS(cdWB_CMR0, cols, ops, vals, maxAgeInSamples)#, maxIndexByCohort = 100)
  fileName = paste0("eh_", stri_paste_list(vals, collapse = "_"))
...
s2 <- data.frame(s) %>%
  unite("all", sep = "_", na.rm = TRUE, remove = TRUE) %>%
  #distinct() %>%
  arrange()

tableS2 <- table(s2)

# of unique section histories
length(tableS2)

# Heatmap of sections - all cohorts
dSectionRiverN <- eh$data %>%
  filter(enc == 1) %>% #will get all transitions, not just consecutive samples
  addLaggedSectionRiverN() %>%
  group_by(sectionRiverN, lagSectionRiverN) %>%
  summarise(n = n())

ggplot(dSectionRiverN, aes(lagSectionRiverN, sectionRiverN, fill = log(n))) + 
  geom_tile() +
  scale_fill_viridis(discrete=FALSE) 
 # theme_ipsum()


# Heatmap of sections - by cohort
dSectionRiverN_cohort <- eh$data %>%
  filter(enc == 1) %>% #will get all transitions, not just consecutive samples
  addLaggedSectionRiverN() %>%
  group_by(cohort, sectionRiverN, lagSectionRiverN) %>%
  summarise(n = n())

ggplot(dSectionRiverN_cohort, aes(lagSectionRiverN, sectionRiverN, fill = log(n))) + 
  geom_tile() +
  scale_fill_viridis(discrete=FALSE) +
  facet_wrap(~ cohort)
 # theme_ipsum()

# Heatmap of sections - by cohort and species
dSectionRiverN_cohort_species <- eh$data %>%
  filter(enc == 1) %>% #will get all transitions, not just consecutive samples
  addLaggedSectionRiverN() %>%
  group_by(species, cohort, sectionRiverN, lagSectionRiverN) %>%
  summarise(n = n())

ggplot(dSectionRiverN_cohort_species %>% filter(species != "ats"), aes(lagSectionRiverN, sectionRiverN, fill = log(n))) + 
  geom_tile() +
  scale_fill_viridis(discrete=FALSE) +
  facet_grid(species~ cohort)
 # theme_ipsum()
```
