...
getEHDataWide <- function(d, cols, ops, vals, var, valueFill = 0){   
  out <- d %>%
...
getEH <- function(d, cols, ops, vals){
  encWide <- getEHDataWide(d, cols, ops, vals, "enc")
  eh <- as.matrix(encWide %>% select(-tag), nrow = nrow(encWide), ncol = ncol(encWide) - 1)
  
  flowWide <- getEHDataWide(d, cols, ops, vals, "meanFlow")
  flowMatrix <- as.matrix(flowWide %>% select(-tag), nrow = nrow(flowWide), ncol = ncol(flowWide) - 1)
  
  temperatureWide <- getEHDataWide(d, cols, ops, vals, "meanTemperature")
  ...
  return(list(eh = eh, flow = flowMatrix, temperature = temperatureMatrix, tags = tags, data = data ))
}
...
if (getNew_encounterHistories) {
  ...
  eh <- getEH(cdWB_CMR0, cols, ops, vals)
