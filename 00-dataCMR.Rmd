...
getEH_AIS <- function(dIn, cols, ops, vals, maxOccasionValue){
  ...
  flowWide <- getEHDataWide_AIS(d, cols, ops, vals, "meanFlowScaled", maxOccasionValue, valuesFill = NA)
  flowMatrix <- as.matrix(flowWide %>% select(-tag), nrow = nrow(flowWide), ncol = ncol(flowWide) - 1)
  
  temperatureWide <- getEHDataWide_AIS(d, cols, ops, vals, "meanTemperatureScaled", maxOccasionValue, valuesFill = NA)
  ...
  riverMatrix <- as.matrix(riverWide %>% select(-tag), nrow = nrow(riverWide), ncol = ncol(riverWide) - 1)
  ...
  last <- apply(eh, 1, function(x) max(which(x != 0)))
  
  return(list(eh = eh, flow = flowMatrix, temperature = temperatureMatrix, river = riverMatrix, 
              tags = tags, cohorts = cohorts, seasons = seasons, first = first, last = last, data = data ))
}
...
if (getNew_encounterHistories) {
  ...
  eh <- getEH_AIS(cdWB_CMR0, cols, ops, vals, maxOccasionValue = maxAgeInSamples)
