## Flow effects on survival (phi) models {#modelNimble}
...
if(plotMCMCOutput) {
  ...
  MCMCsummary(object = mcmc.phiT_pT_cohort_flow, round = 2)
  #MCMCplot(object = mcmc.phiT_pT_cohort_flow, params = "betaPhiOut")
  MCMCplot(object = mcmc.phiT_pT_cohort_flowCohort, params = "betaFlow")# 
  MCMCplot(object = mcmc.phiT_pT_cohort_flowCohort, params = c("betaPhiCohortOut"))
  MCMCplot(object = mcmc.phiT_pT_cohort_flowCohort, params = c("betaPCohortOut"))
  
  priors <- runif(nIter * nChains, 0, 1)
  MCMCtrace(object = mcmc.phiT_pT_cohort_flowCohort,
            #ISB = FALSE,
            #exact = TRUE, 
            params = c("betaPhiCohortOut"),
            pdf = FALSE, 
...
if (rerunSurivalModels) {
  ...
  hmm.phiT_pT_cohort_flowCohortHier <- nimbleCode({
    ...
    for (c in 1:nCohorts){
      ...
      betaPhiCohort[c] ~ dnorm(0,1)
      betaPCohort[c] ~ dnorm(0,1)
      betaFlowCohort[1,c] ~ dnorm(0,1)
      betaFlowCohort[2,c] ~ dnorm(0,1)
      for (t in 1:(T-1)){ 
        betaPhi[t,c] ~ dnorm(betaPhiCohort[c], 1)
        betaP[t,c] ~ dnorm(betaPCohort[c], 1)
      }
    ...
    for (c in 1:nCohorts){
        betaPhiCohortOut[c] <- 1/(1 + exp(-betaPhiCohort[c]))
        betaPCohortOut[c] <- 1/(1 + exp(-betaPCohort[c]))
        betaFlowCohortOut[1,c] <- 1/(1 + exp(-betaFlowCohort[1,c]))
        betaFlowCohortOut[2,c] <- 1/(1 + exp(-betaFlowCohort[2,c]))
      for (t in 1:(T-1)){ 
        betaPhiOut[t,c] <- 1/(1 + exp(-betaPhi[t,c]))
        betaPOut[t,c] <- 1/(1 + exp(-betaP[t,c])) 
      }
  ...
  parametersToSave <- c("betaInt", 
                        ...
                        "betaPhiOut", "betaPOut", "betaPhiCohortOut", "betaPCohortOut", 
                        "betaFlow",
                        "betaFlowCohort", "betaFlowCohortOut")  
  nIter <- 20000
  nBurnin <- 10000
  nChains <- 2
  thinRate <- 5
...
if(plotMCMCOutput) {
  ...
  MCMCplot(object = mcmc.phiT_pT_cohort_flowHierCohort, params = "betaFlow")# 
  MCMCplot(object = mcmc.phiT_pT_cohort_flowHierCohort, params = c("betaPhiCohortOut"))
  MCMCplot(object = mcmc.phiT_pT_cohort_flowHierCohort, params = c("betaPCohortOut"))
  
  priors <- runif(nIter * nChains, 0, 1)
  MCMCtrace(object = mcmc.phiT_pT_cohort_flowHierCohort,
            #ISB = FALSE,
            #exact = TRUE, 
            params = c("betaPhiCohortOut"),
            pdf = FALSE, 
...
ggplot(predFlowCohortHier %>% filter(season ==1), aes(flow, predPhi, group = iter)) +
  geom_line(alpha = 0.1) +
  facet_wrap(~ cohort)
