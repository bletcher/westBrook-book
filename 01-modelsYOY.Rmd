...
AICs and Relative importance of main effects models
...
ggplot(tmp, aes(yday, flow)) +
  geom_point(aes(yday, observedLength/20), color = 'lightblue', data = tmp2) +
  geom_point() +
  scale_x_continuous(breaks = seq(0,300, 30)) +
  facet_wrap(~year)

ggplot(tmp, aes(yday, cumulFlow / 10)) +
  geom_point(aes(yday, observedLength / 20), color = 'lightblue', data = tmp2) +
  geom_point(color = 'darkgrey') +
  geom_point(aes(yday, cumulTemp / 800), color = "orange", data = tmp) +
  geom_point(aes(yday, flow), data = tmp) +
  scale_x_continuous(breaks = seq(0, 300, 30)) +
  facet_wrap(~year)

ggplot(tmp, aes(yday, cumulFlow / 10, color = factor(year))) +
  #geom_point(aes(yday, observedLength / 20), color = 'lightblue', data = tmp2) +
  geom_point() +
