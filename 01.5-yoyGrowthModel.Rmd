...
```{r}
library(tidyverse)
load(file = './data/cdWB.RData')
```

Focus on ageInSample == 1 fish for yoy growth model

```{r AIS1}

  cdWB1 <- cdWB %>% filter(ageInSamples == 1, species != 'ats', riverOrdered == "west brook")
  cd1 <- cdWB %>% filter(ageInSamples == 1, species != 'ats')
...
Why are there untagged fish bigger than 60mm? Check 2002/bkt/WB, as an
example
